...
cell_types <- c("Microglia", "Astrocytes", "Oligodendrocytes", "Excitatory Neurons", "Inhibitory Neurons", "Oligodendrocyte Precursors")
...
### Gene Set Enrichment Analysis (GO and KEGG) and Volcano Plots
...
gse_yvav_graph <- ggplot(gse_yvav_df, aes(NES, fct_reorder(Description, NES), fill = NES > 0)) + 
    ...
    scale_fill_manual(values = c("TRUE" = "#004E89", "FALSE" = "#C19A00")) +
...
# YVAV
keyvals_yvav <- ifelse(
    dge_results[["Microglia"]]$mast_yvav$p_val_adj < .05 &        dge_results[["Microglia"]]$mast_yvav$avg_log2FC < -.5, '#C19A00',
      ifelse(dge_results[["Microglia"]]$mast_yvav$p_val_adj < .05 &        dge_results[["Microglia"]]$mast_yvav$avg_log2FC > .5, '#004E89',
        'gray'))
  keyvals_yvav[is.na(keyvals_yvav)] <- 'gray'
  names(keyvals_yvav)[keyvals_yvav == '#C19A00'] <- 'downregulated'
  names(keyvals_yvav)[keyvals_yvav == '#004E89'] <- 'upregulated'
...
# AMAV
keyvals_amav <- ifelse(
    dge_results[["Microglia"]]$mast_amav$p_val_adj < .05 &        dge_results[["Microglia"]]$mast_amav$avg_log2FC < -.5, '#C19A00',
      ifelse(dge_results[["Microglia"]]$mast_amav$p_val_adj < .05 &        dge_results[["Microglia"]]$mast_amav$avg_log2FC > .5, '#004E89',
        'gray'))
  keyvals_amav[is.na(keyvals_amav)] <- 'gray'
  names(keyvals_amav)[keyvals_amav == '#C19A00'] <- 'downregulated'
  names(keyvals_amav)[keyvals_amav == '#004E89'] <- 'upregulated'
...
ggsave("/stor/work/Fonken/UT23_snRNAseq/local_ut23/Graphs/ut23_volcanogsea.png", dpi = 300, width = 12, height = 15)
...
micro_volcano <- dge_results[["Microglia"]]$volcano_plot
...
AverageExpression(ut23.object, features = choroid_genes, group.by = "sample_id")

VlnPlot(ut23.object, features = choroid_genes, group.by = "sample_id", pt.size = 0.1, combine = TRUE)
n_groups <- length(unique(ut23.object$sample_id))
colors <- colorRampPalette(RColorBrewer::brewer.pal(8, "Set1"))(n_groups)

# Create DotPlot with enough colors

DotPlot(ut23.object, "Ttr", assay="RNA")
